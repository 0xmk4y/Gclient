...
export default function LoginModal() {
    ...
    const handleLogin = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        await signIn('credentials', { email, password, callbackUrl: '/user/dashboard' });
    };
    ...
    return (
        <div className='bg-white text-black border p-4 absolute z-50' style={{ top: '50px', right: '200px', width: '400px' }}>
            ...
            {login && (
                <div>
                    ...
                    <Link href={"#"} className='flex justify-center mt-4 border border-primary px-8 py-2 text-sm gap-2 w-full'>
                        ...
                        <p onClick={() => signIn('github', { callbackUrl: '/user/dashboard' })} className='text-primary font-bold'>Login using Google</p>
                    </Link>
                    <p className='text-center my-4'>or</p>
                    <form onSubmit={handleLogin} className='flex flex-col gap-4'>
                        <div className='border flex items-center px-2 py-2'>
                            <Mail size={20} />
                            <input type="text" name="email" className='focus:outline-none px-2 w-full' placeholder='Email' />
                        </div>
                        <div className='border flex items-center px-2 py-2'>
                            <Lock size={20} />
                            <input type="password" name="password" className='focus:outline-none px-2 w-full' placeholder='Password' />
                        </div>
            ...
            {forgot && (
                <div>
                    <h4 className='font-bold text-center text-[30px]'>Forgot password</h4>
